		...
		<div class="postList">
			<table>
				<c:forEach items="${placeList}" var="dto">
					<tr>
						<td><img src="${path}/resources/image/cafe.jpg" /></td>
						<td>
		...
		<ul class="place_page">
				...
				<li><c:choose>
						<c:when test="${i==pv.currentPage}">
							<a class="page-link page-item active"
								href="placelist.do?currentPage=${i}">${i}</a>
						</c:when>
						<c:otherwise>
							<a class="page-link" href="placelist.do?currentPage=${i}">${i}</a>
						</c:otherwise>
					</c:choose></li>
			</c:forEach>
			<!-- 페이지 출력 끝 -->
			...
			<c:if test="${pv.endPage<pv.totalPage}">
				<li><a class="page-link"
					href="placelist.do?currentPage=${pv.startPage+pv.blockPage}">Next</a></li>
			</c:if>
			<!-- 다음 출력 끝 -->
		</ul>
	</div>
